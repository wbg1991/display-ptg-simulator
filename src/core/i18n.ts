...
const en = {
  ...
  "scan.zone.active": "Active",
  "scan.legend.title": "What are these colored zones?",
  "scan.legend.mnemonic": "Front = before the sync pulse, Back = after it. Both are just idle margins around the pulse: the beam is running but nothing is drawn (black).",
  "scan.legend.frontPorch": "Front Porch - the visible part of the line is over, but the beam keeps going a bit before the sync pulse. A safety margin after the last pixel so the signal can settle.",
  "scan.legend.hSync": "H-Sync - short pulse at the end of EVERY line: \"go back to the left edge, start a new line.\"",
  "scan.legend.backPorch": "Back Porch - the beam is back at the left edge but waits a moment to stabilise before pixel 1 of the next line.",
  "scan.legend.vSync": "V-Sync - the same idea per FRAME: a band of whole lines at the bottom (all blank), then the beam jumps back to the top-left.",
  "scan.legend.hint": "Hover an item to highlight its zone in the grid.",
  "scan.blankingNote": "Porch and sync zones are drawn larger than real proportions so they are visible.",
  "scan.play": "Play",
  "scan.pause": "Pause",
  "scan.reset": "Reset",
...
const ko: typeof en = {
  ...
  "scan.zone.active": "Active",
  "scan.legend.title": "색깔 구간은 뭔가요?",
  "scan.legend.mnemonic": "Front = 싱크 펄스 '앞', Back = 싱크 펄스 '뒤'. 둘 다 싱크 주변의 여유 구간일 뿐입니다: 빔은 계속 움직이지만 아무것도 그리지 않습니다(검정).",
  "scan.legend.frontPorch": "프론트 포치 - 한 줄의 보이는 부분은 끝났지만, 싱크 펄스가 나오기 전까지 빔이 조금 더 갑니다. 마지막 픽셀 뒤의 안전 여유로, 신호가 안정되도록 합니다.",
  "scan.legend.hSync": "H-싱크 - 매 줄 끝마다 나오는 짧은 펄스: \"왼쪽 끝으로 돌아가서 새 줄을 시작하라.\"",
  "scan.legend.backPorch": "백 포치 - 빔이 왼쪽 끝으로 돌아왔지만, 다음 줄의 첫 픽셀을 그리기 전에 잠시 안정화되기를 기다립니다.",
  "scan.legend.vSync": "V-싱크 - 같은 원리를 프레임 단위로: 화면 아래쪽에 줄 전체가 빈 띠로 붙어 있고, 그 뒤 빔이 좌상단으로 점프합니다.",
  "scan.legend.hint": "항목에 마우스를 올리면 격자에서 해당 구간이 강조됩니다.",
  "scan.blankingNote": "포치/싱크 구간은 눈에 보이도록 실제 비율보다 크게 그렸습니다.",
  "scan.play": "재생",
  "scan.pause": "일시정지",
  "scan.reset": "초기화",
  "scan.stepPixel": "+1 픽셀",
